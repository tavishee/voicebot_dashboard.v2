...
cc_data AS (
    SELECT * FROM glue_catalog.recent_search_partition.enser_callback_data c
    WHERE service IN ('Fresh_Car', 'Renewal_Car', 'four_wheeler')
      ...
      AND date >= DATE_FORMAT(DATE_SUB(CURRENT_DATE(), INTERVAL 110 DAY), '%Y%m%d')
      AND created_on >= DATE_SUB(CURRENT_DATE(), INTERVAL 110 DAY)
      AND LOWER(agent) NOT IN ('no agent')
),
daily_calls AS (
    SELECT CAST(c.customer_id AS VARCHAR) AS customer_id, c.agent,
        DATE(c.created_on) AS call_date,
        SUM(IFNULL(CAST(NULLIF(SPLIT_PART(c.talk_duration, ':', 1), '') AS INT), 0) * 3600 +
            IFNULL(CAST(NULLIF(SPLIT_PART(c.talk_duration, ':', 2), '') AS INT), 0) * 60 +
            IFNULL(CAST(NULLIF(SPLIT_PART(c.talk_duration, ':', 3), '') AS INT), 0)) AS daily_talk_seconds,
...
attribution_ranking AS (
    SELECT cc.*,
        CASE WHEN cc.cumulative_talk_seconds >= 30 AND cc.purchase_date IS NOT NULL
          ...
          AND DATEDIFF(cc.purchase_date, cc.call_date) BETWEEN 0 AND 45 THEN 1 ELSE 0 END AS attribution_eligible,
