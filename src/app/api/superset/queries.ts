...
policy_purchases AS (
    ...
    JOIN (
        ...
        WHERE modified_on >= DATE_SUB(CURRENT_DATE(), INTERVAL 60 DAY)
          AND date >= DATE_FORMAT(DATE_SUB(CURRENT_DATE(), INTERVAL 60 DAY), '%Y%m%d')
    ) od ON oi.oms_order_id = od.oms_order_id AND oi.rn = 1 AND od.rn = 1
    JOIN (
        ...
        WHERE modified_on >= DATE_SUB(CURRENT_DATE(), INTERVAL 60 DAY)
          AND date >= DATE_FORMAT(DATE_SUB(CURRENT_DATE(), INTERVAL 60 DAY), '%Y%m%d')
    ) p ON p.proposal_id = od.proposal_id AND p.rn = 1
...
cc_data AS (
    ...
    WHERE c.service IN ('Fresh_Car', 'Renewal_Car', 'four_wheeler')
      ...
      AND CAST(c.customer_id AS VARCHAR) IN (SELECT lead_id FROM qualified_leads)
),
daily_calls AS (
