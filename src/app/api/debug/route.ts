...
export async function GET(request: Request) {
  try {
    ...
    const res = await gmail.users.messages.list({
      userId: 'me',
      q: `from:customreports@greylabs.ai after:${Math.floor(after.getTime()/1000)} before:${Math.floor(before.getTime()/1000)}`,
      maxResults: 5,
    });
    if (!res.data.messages?.length) return NextResponse.json({ error: 'No emails found' });
    ...
    if (xlsxAtt) {
      const att = await gmail.users.messages.attachments.get({
        userId: 'me', messageId: res.data.messages[0].id!, id: xlsxAtt.attachmentId,
      });
      const buffer = Buffer.from(att.data.data, 'base64');
      const wb = XLSX.read(buffer, { type: 'buffer' });
      sheetInfo = { sheetNames: wb.SheetNames, sheets: {} as any };
      for (const name of wb.SheetNames) {
        const ws = wb.Sheets[name];
        const rows: any[][] = XLSX.utils.sheet_to_json(ws, { header: 1 });
        sheetInfo.sheets[name] = { rowCount: rows.length, headers: rows[0] || [], sample: rows.slice(1,3) };
      }
    }
