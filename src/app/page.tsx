  ...
  const syncSuperset=async()=>{
    ...
    try{
      ...
      if(extensionReady){
        ...
        const idValues=allIds.map((id:string)=>`('${id.replace(/'/g,"''")}')`).join(',');
        const sql=`WITH qualified_leads AS (SELECT CAST(id AS VARCHAR) AS lead_id FROM (VALUES ${idValues}) AS t(id)),raw_calls AS (SELECT CAST(customer_id AS VARCHAR) AS customer_id,disposition1,disposition2,disposition3,talk_duration FROM glue_catalog.recent_search_partition.enser_callback_data WHERE (source='enser' OR source IS NULL) AND customer_id<>'NA' AND date>=DATE_FORMAT(CAST('${ssDate}' AS DATE),'%Y%m%d') AND date<DATE_FORMAT(CAST('${nextDate}' AS DATE),'%Y%m%d') AND created_on>='${ssDate} 00:00:00' AND created_on<'${nextDate} 00:00:00' AND CAST(customer_id AS VARCHAR) IN (SELECT lead_id FROM qualified_leads))SELECT COUNT(*) AS cc_sent,SUM(CASE WHEN COALESCE(disposition1,'')<>'' OR COALESCE(disposition2,'')<>'' OR COALESCE(disposition3,'')<>'' THEN 1 ELSE 0 END) AS cc_attempted,SUM(CASE WHEN IFNULL(CAST(NULLIF(SPLIT_PART(talk_duration,':',1),'') AS INT),0)*3600+IFNULL(CAST(NULLIF(SPLIT_PART(talk_duration,':',2),'') AS INT),0)*60+IFNULL(CAST(NULLIF(SPLIT_PART(talk_duration,':',3),'') AS INT),0)>0 THEN 1 ELSE 0 END) AS cc_connected,0 AS cc_converted FROM raw_calls`;
